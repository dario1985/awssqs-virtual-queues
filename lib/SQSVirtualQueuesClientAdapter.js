...
class SQSVirtualQueuesClient extends SQSClientAdapter_1.SQSClientAdapter {
    ...
    getVirtualQueue(queueName) {
        var _a;
        return (_a = this.virtualQueues.get(queueName)) !== null && _a !== void 0 ? _a : null;
    }
...
class HostQueue {
    constructor(sqs, queueUrl) {
        ...
        this.debug = (0, debug_1.default)(`SQSVirtualQueuesClient:HostQueue[*${queueUrl.slice(-5)}]`);
    ...
    async receiveMessage(maxMessages, waitTimeMs) {
        ...
        while (!this.closed && Date.now() < deadlineWaitMs && result.length < maxMessages) {
            ...
            if (result.length < maxMessages) {
                await (0, utils_1.wait)((0, utils_1.clamp)(deadlineWaitMs - Date.now(), 0, waitTimeMs));
            }
        }
        return result;
    }
...
class UnhandledVirtualQueueError extends errors_1.QueueError {
